/* -------------------CHANGELOG--------------------
3.2.1.SCS
 - To use with "hostname" instead of "sn_main_name" by Sinaloense
...
public OnPluginStart()
{
	...
	HookConVarChange(cvarHostname, OnHostnameChanged);
...
public OnHostnameChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	if(StrEqual(oldVal, "[Hostname #0]") || StrEqual(oldVal, "Hostname"))
	{
		UnhookConVarChange(cvarHostname, OnHostnameChanged);
		ServerCommand("sn_main_name %s", newVal);
	}
}
